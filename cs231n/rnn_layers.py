...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        tdx, tdprev_h, tdWx, tdWh, tdb = rnn_step_backward(dh[:, t, :] + tdprev_h, cache[t])
        dx.append(tdx)
        dWx += tdWx
        dWh += tdWh
        db += tdb
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h, cache = [], []
    for i in range(T):
        next_h, next_c, step_cache = lstm_step_forward(x[:, i, :], h0, c0, Wx, Wh, b)
        cache.append(step_cache)
        h.append(next_h)
        h0, c0 = next_h, next_c
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        tdx, dprev_h, dprev_c, tdWx, tdWh, tdb = lstm_step_backward(
